...
defined( 'TF_PATH' ) or define( 'TF_PATH', TP_FRAMEWORK_LIBS_DIR . 'titan-framework/' );
...
//		add_action( 'plugins_loaded', array( $this, 'loadTextDomain' ) );
	...
	public function loadTextDomain() {
		load_plugin_textdomain( TF_I18NDOMAIN, false, basename( get_template_directory() ) . '/framework/libs/titan-framework/languages/' );
	}
	...
	public function pluginLinks( $plugin_meta, $plugin_file ) {
		if ( $plugin_file == plugin_basename( __FILE__ ) ) {
			$plugin_meta[] = sprintf( "<a href='%s' target='_blank'>%s</a>", "http://www.titanframework.net/docs", __( "Documentation", 'thim-framework' )
			);
			$plugin_meta[] = sprintf( "<a href='%s' target='_blank'>%s</a>", "https://github.com/gambitph/Titan-Framework", __( "GitHub Repo", 'thim-framework' )
			);
			$plugin_meta[] = sprintf( "<a href='%s' target='_blank'>%s</a>", "https://github.com/gambitph/Titan-Framework/issues", __( "Issue Tracker", 'thim-framework' )
			);
		}
		return $plugin_meta;
	}
