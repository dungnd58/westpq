...
class TitanFrameworkOptionRadioImage extends TitanFrameworkOption {
	...
	public function display() {
		...
		foreach ( $this->settings['options'] as $key => $imageURL ) {
			...
			printf( '<label id="%s"><input id="%s" type="radio" name="%s" value="%s" %s/> <img src="%s" /></label>', $this->getID() . $key, $this->getID() . $key, $this->getID(), esc_attr( $key ), checked( $value, $key, false ), esc_attr( $imageURL )
			);
		}
		...
		public function render_content() {
			...
			if ( $this->id == 'thim_header_style' ) {
				...
				foreach ( $this->choices as $key => $imageURL ) {
					...
					$image_thumb_index = strrpos( $imageURL, '.', - 1 );
					$image_thumb       = substr_replace( $imageURL, '_thumb', $image_thumb_index, 0 );
					?>
					<label class='tf-radio-image'>
						<input type="radio" name="<?php echo esc_attr( $this->id ) ?>" value="<?php echo esc_attr( $key ) ?>" <?php $this->link();
						checked( $value, $key );
						?>/>
						<img src="<?php echo esc_attr( $image_thumb ) ?>" class="of-radio-img-header" rel-data="<?php echo ( ( $i - 1 ) % 6 ) ?>'"/>
						<img src="<?php echo esc_attr( $imageURL ) ?>" alt="" style="display:none;position:absolute;" class="header_preview"/>
					</label>
				<?php
				}
