...
class TitanFrameworkOptionCode extends TitanFrameworkOption {
	...
	public function printJSForPagesAndPosts() {
		...
		?>
		<script>
		<?php echo ent2ncr($this->getFramework()->getOption( $this->settings['id'], $id )) ?>
		</script>
		<?php
	}
	...
	public function printCSSForPagesAndPosts() {
		...
		require_once( trailingslashit( get_template_directory() ) . "framework/libs/titan-framework/inc/scssphp/scss.inc.php" );
		$scss = new scssc();
		try {
	...
	public function display() {
		...
		jQuery(document).ready(function ($) {
			var container = jQuery('#<?php echo esc_js($this->getID()) ?>_ace_editor');
			container.width( container.parent().width() ).height( <?php echo esc_js($this->settings['height']) ?> );

			var editor = ace.edit( "<?php echo esc_js($this->getID()) ?>_ace_editor" );
			container.css('width', 'auto');
			editor.setValue(container.siblings('textarea').val());
			editor.setTheme("ace/theme/<?php echo esc_js($this->settings['theme']) ?>");
			editor.getSession().setMode('ace/mode/<?php echo esc_js($this->settings['lang']) ?>');
...
function registerTitanFrameworkOptionCodeControl() {
	class TitanFrameworkOptionCodeControl extends WP_Customize_Control {
		...
		public function render_content() {
			...
			<script>
			jQuery(document).ready(function ($) {
				var container = jQuery('#<?php echo esc_js($this->id) ?>_ace_editor');
				container.width( container.parent().width() ).height( <?php echo esc_js($this->height )?> );

				var editor = ace.edit( "<?php echo esc_js($this->id) ?>_ace_editor" );
				container.css('width', 'auto');
				editor.setValue(container.siblings('textarea').val());
				editor.setTheme("ace/theme/<?php echo esc_js($this->theme) ?>");
				editor.getSession().setMode('ace/mode/<?php echo esc_js($this->lang) ?>');
				editor.setShowPrintMargin(false);
				editor.setHighlightActiveLine(false);
				editor.gotoLine(1);
				editor.session.setUseWorker(false);
			...
			// The hidden textarea that will hold our contents
			?><textarea <?php $this->link(); ?> style='display: none'><?php echo ent2ncr($this->value()) ?></textarea><?php
