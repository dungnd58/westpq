...
if ( class_exists( 'WP_Importer' ) ) {
	...
	class WP_Import extends WP_Importer {
		...
		function import_options() {
			...
			<form action="<?php echo admin_url( 'admin.php?import=wordpress&amp;step=2' ); ?>" method="post">
				...
				<?php if ( ! empty( $this->authors ) ) : ?>
					<h3><?php esc_attr_e( 'Assign Authors', 'thim-framework' ); ?></h3>
					<p><?php esc_attr_e( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', 'thim-framework' ); ?></p>
				...
				<?php if ( $this->allow_fetch_attachments() ) : ?>
					<h3><?php esc_attr_e( 'Import Attachments', 'thim-framework' ); ?></h3>
					<p>
						<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments"/>
						<label for="import-attachments"><?php esc_attr_e( 'Download and import file attachments', 'thim-framework' ); ?></label>
					</p>
				<?php endif; ?>

				<p class="submit">
					<input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'thim-framework' ); ?>"/>
				</p>
			</form>
			<?php
		}
		...
		function author_select( $n, $author ) {
			...
			if ( $create_users ) {
				if ( $this->version != '1.0' ) {
					esc_attr_e( 'or create new user with login name:', 'thim-framework' );
					$value = '';
				} else {
					esc_attr_e( 'as a new user:', 'thim-framework' );
			...
			if ( ! $create_users && $this->version == '1.0' ) {
				esc_attr_e( 'assign posts to an existing user:', 'thim-framework' );
			} else {
				esc_attr_e( 'or assign posts to an existing user:', 'thim-framework' );
			}
			wp_dropdown_users( array(
				'name'            => "user_map[$n]",
				'multi'           => true,
				'show_option_all' => __( '- Select -', 'thim-framework' )
			) );
		...
		function process_menu_item( $item ) {
			...
			foreach ( $item['postmeta'] as $meta ) {
				...
				do_action( 'import_menu_item_meta', $menu_id, $meta['key'], $meta['value'] );
			}
		...
		function process_attachment( $post, $url ) {
			if ( ! $this->fetch_attachments ) {
				return new WP_Error( 'attachment_processing_error', __( 'Fetching attachments is not enabled', 'thim-framework' ) );
			}
			...
			if ( ! $this->method ) {
				...
				if ( ! $attachment_exist ) {
					...
					if ( $info = wp_check_filetype( $upload['file'] ) ) {
						...
						return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'thim-framework' ) );
					}
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( ! $headers ) {
				...
				return new WP_Error( 'import_file_error', __( 'Remote server did not respond', 'thim-framework' ) );
			}
			...
			if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
				...
				return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'thim-framework' ) );
			}

			if ( 0 == $filesize ) {
				@unlink( $upload['file'] );

				return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'thim-framework' ) );
			}
			...
			if ( ! empty( $max_size ) && $filesize > $max_size ) {
				...
				return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s', 'thim-framework' ), size_format( $max_size ) ) );
			}
		...
		function remap_featured_images() {
			...
			if ( $attachment_id ) {
				$wpdb->query( $wpdb->prepare( "UPDATE $wpdb->postmeta
						SET meta_value = %d
							WHERE meta_key = %s AND meta_value > %d", $attachment_id, '_thumbnail_id', 0 ) );
			}

		}

		private function save_mapping() {
			update_option( 'thim_import_mapping_terms', $this->processed_terms );
			update_option( 'thim_import_mapping_posts', $this->processed_posts );
		}
		...
		function header() {
			echo '<h2>' . __( 'Import WordPress', 'thim-framework' ) . '</h2>';
		...
		function pn_get_attachment_id_from_url( $attachment_url = '' ) {
			...
			if ( false !== strpos( $attachment_url, $upload_dir_paths['baseurl'] ) ) {
				...
				$attachment_id = $wpdb->get_var( $wpdb->prepare( "SELECT wposts.ID FROM $wpdb->posts as wposts, $wpdb->postmeta as wpostmeta
							WHERE wposts.ID = wpostmeta.post_id
							AND wpostmeta.meta_key =  %s
							AND wpostmeta.meta_value = %s
							AND wposts.post_type = %s", '_wp_attached_file', $attachment_url, 'attachment' ) );
...
if ( ! function_exists( 'wordpress_importer_init' ) ) {
	...
	function wordpress_importer_init() {
		...
		register_importer( 'wordpress', 'WordPress', __( 'Import <strong>posts, pages, comments, custom fields, categories, and tags</strong> from a WordPress export file.', 'thim-framework' ), array(
			$GLOBALS['wp_import'],
			'dispatch'
		) );
	}
