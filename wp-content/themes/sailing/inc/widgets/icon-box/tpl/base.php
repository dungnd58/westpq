...
if ($instance['widget_background'] == 'bg_video' && $instance['self_video'] != '') {
    ...
    $muted = (isset($instance['video_group']['muted']) && $instance['video_group']['muted'] === 'false') ? '' : 'muted';
    $loop = (isset($instance['video_group']['loop']) && $instance['video_group']['muted'] === 'false') ? '' : 'loop';
    $auto_play = (isset($instance['video_group']['auto_play']) && $instance['video_group']['auto_play'] === 'false') ? 'false' : 'true';
    $bg_video = '<video ' . $loop . ' ' . $muted . ' class="full-screen-video" data-autoplay="' . $auto_play . '">
					  <source src="' . $src . '" type="video/mp4">
  				</video>';
    if ($muted === '') {
        $bg_video .= '<span class="mute-audio"></span>';
    }
...
$auto_hidden = (isset($instance['video_group']['auto_hidden']) && $instance['video_group']['auto_hidden'] === 'false') ? '' : 'auto-hidden';

$prefix = '<div class="inner-box ' . $auto_hidden . '">';
if ($instance['self_poster'] != '') {
    $background_video = 'style="background-image:url(' . wp_get_attachment_url($instance['self_poster']) . ')"';
} else {
    $background_video = '';
}
$prefix .= '<div ' . $background_video . ' class="wrapper-box-icon' . $extent_class . $class_bg_video . ' ' . $instance['layout_group']['text_align_sc'] . ' ' . $instance['layout_group']['style_box'] . ' ' . $instance['layout_group']['box_icon_style'] . $thim_animation . '" ' . $data_color . '>';
$suffix = '</div>';
//wrapper-box-icon
...
if ($instance['icon_type'] == 'font-awesome') {
    ...
    if ($instance['icon_type'] == 'font-7-stroke') {
        ...
        $html_icon .= $link_prefix;
        $html_icon .= thim_feature_image($instance['font_image_group']['icon_img']);
        $html_icon .= $link_sufix;
        $html_icon .= '</span></span>' . $complete_suffix . '</div>';
    }
}
...
if ($instance['desc_group']['content'] != '') {
    ...
    $html_content .= ($instance['desc_group']['content'] != '') ? '<div ' . $color_desc . '>' . $complete_prefix . $instance['desc_group']['content'] . $complete_suffix . '</div>' : '';
